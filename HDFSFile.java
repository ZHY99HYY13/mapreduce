...
public class HDFSFile {
	...
	public FileSystem return_FileSystem(){
		return hdfs;
	}
	/**
	 * 构造方法
	...
	public static void main (String arg[]) throws Exception{
		//
		HDFSFile file = new HDFSFile(new Path("hdfs://127.0.0.1:9000/"));
		Path dst = new Path("hdfs://127.0.0.1:9000/test/");
		Path src = new Path("/C:/Users/ZHXY/Desktop/test.txt");
		//创建目录
		file.mkDir(dst);
		//上传本地文件到Hdfs
		...
		//获得修改时间
		file.getModifyTime(new Path(dst+"/test.txt"));
		//删除文件或目录
		file.delFile(dst);
		//新建文件，并写入
		file.writeFile(new Path(dst+"/test.txt"), "我要去深圳");
		//读取目录下所有文件
		file.listFiles(new Path("hdfs://127.0.0.1:9000/test"));
		//读取新写入的文件
		file.readFile(new Path(dst+"/test.txt"));
	}
}
